...
(function (includes) {
  ...
  function renderItem(item) {
    ...
    const indicator = item.active ? '<span class="absolute ml-[-12px] h-7 w-1 rounded-r-full bg-ubis-teal"></span>' : '';
    ...
    return `
      <a href="${escapeHtml(item.href || '#')}" class="nav-link relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] ${activeClasses}">
  ...
  includes.sidebar = function () {
    ...
    return `<aside id="sidebar" class="sidebar fixed left-0 top-1 bottom-0 z-40 w-[238px] border-r border-slate-200 bg-white transition-transform duration-300 lg:translate-x-0">
      <div class="flex h-[82px] items-center border-b border-slate-100 px-4">
      ...
      <div class="absolute bottom-4 left-3 right-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
