...
function renderSharedShell() {
    ...
    if (
        ...
        typeof window.UBISAppIncludes.renderShell === "function"
    ) {
...
function openConfirmDialog(title, message, confirmLabel, onConfirm, danger) {
  ...
  wrap.innerHTML = [
    ...
    '<h2 class="text-xl font-black text-navy dark:text-white">' + escapeHtml(title) + '</h2>',
    '<p class="confirm-copy">' + escapeHtml(message) + '</p>',
    '<div class="confirm-actions">',
    '<button class="btn-secondary" data-cancel>Cancel</button>',
    '<button class="' + (danger ? 'btn-primary btn-danger' : 'btn-primary') + '" data-confirm>' + escapeHtml(confirmLabel) + '</button>',
    '</div>',
    '</div>'
  ].join("");
  ...
  wrap.querySelector("[data-confirm]").addEventListener("click", function () { onConfirm(); wrap.remove(); });
}
...
function initTabs() {
    ...
    const map = [
        ["tab-1-trigger","body-tab1"],
        ["tab-2-trigger","body-tab2"],
        ["tab-3-trigger","body-tab3"]
    ];

    map.forEach(([triggerId, bodyId]) => {
...
function initRadioToggle() {
    ...
    const withPara = document.getElementById("withParaSection");
    const withoutPara = document.getElementById("withoutParaSection");
    // Page doesn't contain radio section
    if (!showRadio || !hideRadio || !withPara || !withoutPara) {
        return;
    }
    function toggle() {
        if (showRadio.checked) {
            withPara.classList.remove("hidden");
            withoutPara.classList.add("hidden");
        } else {
            withPara.classList.add("hidden");
            withoutPara.classList.remove("hidden");
        }
    }
    showRadio.removeEventListener("change", toggle);
    hideRadio.removeEventListener("change", toggle);
    showRadio.addEventListener("change", toggle);
    hideRadio.addEventListener("change", toggle);
    toggle();
}

    (() => {
      const body = document.querySelector('#schemeRows');
      const status = document.querySelector('#sequenceStatus');
      const originalMarkup = body.innerHTML;
      let draggedRow = null;
      const announce = message => { status.textContent = message; };
      const updateNumbers = () => [...body.rows].forEach((row, index) => row.querySelector('[data-number]').textContent = index + 1);
      const clearDropIndicators = () => [...body.rows].forEach(row => row.classList.remove('drop-before', 'drop-after'));
      const moveRow = (row, direction) => {
        const sibling = direction === 'up' ? row.previousElementSibling : row.nextElementSibling;
        if (!sibling) return;
        direction === 'up' ? body.insertBefore(row, sibling) : body.insertBefore(sibling, row);
        updateNumbers();
        announce(`${row.dataset.scheme} moved to position ${[...body.rows].indexOf(row) + 1}.`);
        row.focus();
      };
      const enableRow = row => {
        const cells = row.querySelectorAll(':scope > td');
        if (cells.length < 2) return;
        const name = cells[cells.length - 1].textContent.trim();
        row.classList.add('scheme-row'); row.draggable = true; row.tabIndex = 0; row.dataset.scheme = name;
        row.setAttribute('aria-label', `${name}. Draggable row.`);
        const handleCell = document.createElement('td');
        handleCell.className = 'text-center';
        handleCell.innerHTML = '<span class="drag-handle" aria-hidden="true">&#x2630;</span><span class="sr-only">Drag to reorder</span>';
        row.insertBefore(handleCell, cells[0]);
        cells[0].dataset.number = '';
        row.addEventListener('dragstart', event => { draggedRow = row; row.classList.add('is-dragging'); event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', name); });
        row.addEventListener('dragend', () => { row.classList.remove('is-dragging'); clearDropIndicators(); draggedRow = null; });
        ...
        row.addEventListener('drop', event => {
          event.preventDefault(); if (!draggedRow || draggedRow === row) return;
          const before = event.clientY < row.getBoundingClientRect().top + row.offsetHeight / 2;
          body.insertBefore(draggedRow, before ? row : row.nextSibling); updateNumbers(); clearDropIndicators();
          announce(`${draggedRow.dataset.scheme} moved to position ${[...body.rows].indexOf(draggedRow) + 1}.`);
        });
        row.addEventListener('keydown', event => {
          if (!event.altKey || !['ArrowUp', 'ArrowDown'].includes(event.key)) return;
          event.preventDefault(); moveRow(row, event.key === 'ArrowUp' ? 'up' : 'down');
        });
      };
      const enableAllRows = () => { [...body.rows].forEach(enableRow); updateNumbers(); };
      document.querySelector('#saveSequence').addEventListener('click', () => {
        const sequence = [...body.rows].map(row => row.dataset.scheme);
        // Replace this with your API request, e.g. fetch('/api/schemes/sequence', {method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(sequence)}).
        console.log('Updated scheme sequence:', sequence);
        announce('Scheme sequence is ready to save.');
      });
      document.querySelector('#resetSequence').addEventListener('click', () => { body.innerHTML = originalMarkup; enableAllRows(); announce('The original scheme order has been restored.'); });
      enableAllRows();
    })();
